#include "gpuRadialSensePrepGadgetFCRL.h"
#include "b1_map.h"
...
namespace Gadgetron{
  ...
  gpuRadialSensePrepGadgetFCRL::compute_csm( unsigned int idx )
  {    
  ...
  boost::shared_ptr< hoNDArray<float_complext> > 
  gpuRadialSensePrepGadgetFCRL::compute_reg( unsigned int set, unsigned int slice, bool new_frame )
  {    
  ...
  void 
  gpuRadialSensePrepGadgetFCRL::allocate_accumulation_buffer( unsigned int size )
  {    
  ...
  void gpuRadialSensePrepGadgetFCRL::reconfigure(unsigned int set, unsigned int slice, bool use_dcw)
  {    
    gpuRadialPrepGadgetFCRL::reconfigure(set, slice, use_dcw);
  ...
  GADGET_FACTORY_DECLARE(gpuRadialSensePrepGadgetFCRL)
}
